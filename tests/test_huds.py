...
def test_halo_shield_depletes_entirely_before_health(halo, make_ctx):
    ...
    def lv(d):
        return halo.levels(make_ctx(total_weight=d, full_at=10.0))
...
def test_halo_odd_capacity_health_only_drains_after_shield(halo, make_ctx):
    ...
    _, _, sr, hr = halo.levels(make_ctx(total_weight=6.0, full_at=11.0))
    assert sr == 0.0 and hr == 5.0
    _, _, sr, hr = halo.levels(make_ctx(total_weight=8.0, full_at=11.0))
...
def test_every_hud_honors_max_messages_and_weight_scale(hud_name, qapp, make_ctx):
    ...
    # defaults when the [hud.*] block omits them: capacity = global full_at,
    # drain unscaled.
    assert hud.capacity(make_ctx(full_at=10.0)) == 10.0
    assert hud.damage(make_ctx(total_weight=4.0)) == 4.0
    assert hud.fraction(make_ctx(total_weight=5.0, full_at=10.0)) == 0.5
    # per-HUD max_messages overrides the global capacity for this overlay
    assert hud.capacity(make_ctx(full_at=10.0, params={"max_messages": 20})) == 20.0
    assert hud.fraction(make_ctx(total_weight=5.0, full_at=10.0,
...
def test_every_hud_falls_back_on_invalid_capacity_drain(hud_name, qapp, make_ctx):
    hud = load_huds()[hud_name]
    # garbage / out-of-range -> defaults (0 inherits global full_at; scale 1.0)
    assert hud.capacity(make_ctx(full_at=10.0, params={"max_messages": "lots"})) == 10.0
    assert hud.capacity(make_ctx(full_at=10.0, params={"max_messages": 0})) == 10.0
...
def test_halo_levels_uses_configured_shield_fraction(halo, make_ctx):
    ctx = make_ctx(total_weight=0.0, full_at=10.0, params={"shield_fraction": 0.8})
...
def test_halo_warning_threshold_is_configurable(halo, make_ctx):
    ...
    base = dict(total_weight=7.0, full_at=10.0)
...
def test_halo_health_resolution_param(halo, make_ctx):
    base = dict(total_weight=0.0, full_at=10.0)          # health_units = 5
...
def test_halo_shield_damage_is_clear_not_red(halo, make_ctx):
    ...
    img = render_to_image(halo, w, h,
                          make_ctx(total_weight=4.0, full_at=10.0, w=w, h=h, params=_HP))
...
def test_halo_health_red_below_threshold_cyan_above(halo, make_ctx):
    ...
    low = render_to_image(halo, w, h,
                          make_ctx(total_weight=9.0, full_at=10.0, w=w, h=h, params=_HP))
    c_low = low.pixelColor(x, y)                     # health at 20% -> red
    assert c_low.red() > 150 and c_low.green() < 90
    healthy = render_to_image(halo, w, h,
                              make_ctx(total_weight=6.0, full_at=10.0, w=w, h=h, params=_HP))

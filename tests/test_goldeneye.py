...
def test_shield_depletes_before_health(ge, make_ctx):
    ...
    def lv(d):
        return ge.levels(make_ctx(total_weight=d, max_messages=10.0))
...
def test_levels_honor_shield_fraction(ge, make_ctx):
    s, h = ge.levels(make_ctx(total_weight=0.0, max_messages=10.0,
                              params={"shield_fraction": 0.8}))
    assert (s, h) == (1.0, 1.0)
    # 8 shield / 2 health: 8 damage empties shield exactly, health full
    s, h = ge.levels(make_ctx(total_weight=8.0, max_messages=10.0,
                              params={"shield_fraction": 0.8}))
...
def test_both_arcs_render_when_full(ge, make_ctx):
    img = render_to_image(ge, 260, 230, make_ctx(total_weight=0.0, max_messages=10.0,
                                                 w=260, h=230))
...
def test_shield_arc_empties_before_health_arc(ge, make_ctx):
    ...
    img = render_to_image(ge, 260, 230, make_ctx(total_weight=5.0, max_messages=10.0,
                                                 w=260, h=230))
    warm, blue = _counts(img)

...
def test_lives_total_remaining_and_clamp(mario, make_ctx):
    assert mario.lives(make_ctx(total_weight=0.0, max_messages=10.0)) == (10, 10.0)
    assert mario.lives(make_ctx(total_weight=3.0, max_messages=10.0)) == (10, 7.0)
    assert mario.lives(make_ctx(total_weight=10.0, max_messages=10.0)) == (10, 0.0)
    assert mario.lives(make_ctx(total_weight=15.0, max_messages=10.0)) == (10, 0.0)  # clamp
    assert mario.lives(make_ctx(total_weight=2.5, max_messages=10.0)) == (10, 7.5)   # fractional


def test_lives_total_rounds_max_messages(mario, make_ctx):
    assert mario.lives(make_ctx(max_messages=11.0))[0] == 11
    assert mario.lives(make_ctx(max_messages=0.0))[0] == 1        # never zero slots
...
def test_missing_icon_path_still_renders_bundled(mario, make_ctx):
    img = render_to_image(mario, 600, 96,
                          make_ctx(total_weight=0.0, max_messages=5.0,
...
def test_full_lives_more_opaque_than_fewer(mario, make_ctx):
    w, h = 600, 96
    full = render_to_image(mario, w, h, make_ctx(total_weight=0.0, max_messages=10.0, w=w, h=h, params=_MP))
    half = render_to_image(mario, w, h, make_ctx(total_weight=5.0, max_messages=10.0, w=w, h=h, params=_MP))
    empty = render_to_image(mario, w, h, make_ctx(total_weight=10.0, max_messages=10.0, w=w, h=h, params=_MP))
...
def test_lost_opacity_zero_hides_lost_lives(mario, make_ctx):
    ...
    full = render_to_image(mario, w, h,
                           make_ctx(total_weight=0.0, max_messages=10.0, w=w, h=h, params=hidden))
